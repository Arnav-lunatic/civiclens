    ...
    <div className="relative min-h-[85vh] flex items-center justify-center px-4 py-16 overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl p-8 sm:p-10 border border-slate-200/80 dark:border-slate-800 shadow-2xl shadow-sky-500/5 space-y-6 relative">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Citizen Onboarding</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Join the geo-verified civic redressal network</p>
        </div>
        ...
        {step === 1 ? (
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1.5">Full Name</label>
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1.5">Phone Number (Optional)</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1.5">Set Password</label>
              <div className="relative">
          ...
          <form onSubmit={handleVerifySignup} className="space-y-4">
            <div className="text-center text-xs text-slate-600 dark:text-slate-300">
              OTP code dispatched to <strong className="text-sky-600 dark:text-sky-400">{email}</strong>
            </div>

            {devOtp && (
              <div className="p-3 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-500/20 rounded-2xl text-xs text-center font-medium">
                Development Preview Code: <strong className="font-mono text-sm tracking-widest text-emerald-600 dark:text-emerald-400">{devOtp}</strong>
              </div>
            )}

            <div>
              <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2 text-center">
                Enter 6-Digit Code
              </label>
              <input
                type="text"
                ...
                placeholder="849201"
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-2xl text-lg font-mono text-center tracking-[0.4em] text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-sky-500/50 focus:border-sky-500 focus:outline-none transition"
              />
            </div>
            ...
            <button
              type="button"
              onClick={() => setStep(1)}
              className="w-full text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-center transition"
            >
