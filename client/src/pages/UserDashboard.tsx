    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-slate-900/80 backdrop-blur-xl p-5 rounded-3xl border border-slate-800 shadow-lg space-y-1">
          <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider font-mono">Total Reported</div>
          <div className="text-3xl font-black font-mono text-white">{total}</div>
        </div>
        <div className="bg-slate-900/80 backdrop-blur-xl p-5 rounded-3xl border border-slate-800 shadow-lg space-y-1">
          <div className="text-[11px] font-bold text-amber-400 uppercase tracking-wider font-mono">Under Triage</div>
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl p-5 rounded-3xl border border-slate-800 shadow-lg space-y-1">
          <div className="text-[11px] font-bold text-emerald-400 uppercase tracking-wider font-mono">Resolved</div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-400">Track real-time resolution progress and officer status</p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl p-12 text-center border border-slate-800 shadow-xl space-y-3">
            <CheckCircle2 className="w-12 h-12 text-sky-400 mx-auto" />
            <h3 className="text-lg font-bold text-white">No Complaints Lodged Yet</h3>
            <p className="text-xs text-slate-400 max-w-sm mx-auto">Spotted a civic hazard? Report it with live geotagged proof in seconds.</p>
            <Link
              to="/report"
              className="inline-block mt-2 px-6 py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-teal-500 text-white text-xs font-bold shadow-lg shadow-sky-500/25 hover:from-sky-400 hover:to-teal-400 transition"
            >
              Report Hazard Now
            </Link>
          </div>
            ...
            {complaints.map((item) => {
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div className="p-5 space-y-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-xs font-bold text-sky-400">{item.category}</span>
                        {item.district && (
                      ...
                      <div className="space-y-1.5 pt-1">
                        ...
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-sky-400 hover:text-sky-300 font-bold text-[11px] bg-sky-500/10 hover:bg-sky-500/20 px-2.5 py-1 rounded-lg border border-sky-500/30 transition"
                        >
                  ...
                  <div className="p-5 pt-0 border-t border-slate-800/80 mt-3 pt-3 space-y-2">
                    ...
                    {item.resolvedImageUrl ? (
                      <div className="p-3.5 bg-emerald-950/40 rounded-2xl border border-emerald-500/30 space-y-2.5 shadow-2xs">
                        ...
                        <ComplaintImage
                          ...
                          topLeftBadge={
                            <span className="px-2 py-0.5 rounded bg-emerald-600/90 text-white text-[9px] font-bold shadow backdrop-blur-xs">
                              AFTER: Work Done
                            </span>
                          }
                        />
                        {item.resolutionNotes && (
                          <div className="p-2.5 bg-slate-800/80 rounded-xl border border-emerald-500/20 text-xs text-slate-200">
                            <span className="font-bold text-emerald-400 text-[10px] uppercase block mb-0.5">Officer Resolution Note:</span>
                      ...
                      <div className="p-3 bg-emerald-950/40 rounded-xl border border-emerald-500/30 text-xs text-emerald-300 flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                        <span className="font-semibold">Issue marked as Resolved by municipal authorities.</span>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between text-[11px] text-slate-400 bg-slate-800/50 p-2.5 rounded-xl border border-slate-700/60">
                        <span className="flex items-center gap-1">
                          <Clock className="w-3.5 h-3.5 text-slate-500" />
                          <span>Reported: {new Date(item.createdAt).toLocaleDateString()}</span>
                        </span>
                        <span className="font-bold text-amber-400">Resolution in Progress</span>
                      </div>
                    )}
